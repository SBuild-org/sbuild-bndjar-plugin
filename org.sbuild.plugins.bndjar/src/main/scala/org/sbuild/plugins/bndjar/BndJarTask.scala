 ...
 *
 * Use [[BndJarTask$#apply]] to configure and execute it on one go.
 *
 */
object BndJarTask {

  /**
   * Creates, configure and execute the BndJarTask.
   *
   * For parameter documentation see the [[BndJarTask]] constructor.
   *
   */
 ...
 *
 * To easily configure and execute the BndJarTask task in one go, see [[BndJarTask$#apply]].
 *
 * '''Example:'''
 * {{{
 ...
 *     org.sbuild.plugin.bndjar.BndJarTask(
 ...
 *
 * @constructor
 * Creates a new BndJarTask instance. All parameters can be omitted and set later.
 *
...
class BndJarTask(
  ...
  private[this] val log = Logger[BndJarTask]

  def execute {

    val bndClassLoader = bndClasspath match {
      case null => classOf[BndJarTask].getClassLoader
      case cp =>
