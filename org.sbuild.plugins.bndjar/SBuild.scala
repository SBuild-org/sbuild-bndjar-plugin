...
class SBuild(implicit _project: Project) {
  ...
  val namespace = "org.sbuild.plugins.bndjar"
  val version = "0.0.9000"
  val url = "https://github.com/SBuild-org/sbuild-bndjar-plugin"
  ...
  Plugin[SBuildPlugin] configure (_.copy(
    ...
    pluginClass = s"${namespace}.BndJar",
  ...
  Plugin[MavenDeploy] configure (_.copy(
    ...
    version = version,
    artifactName = Some("SBuild BndJar Plugin"),
    description = Some("An SBuild Plugin that supports creation of OSGi bundle files with the bnd tools."),
